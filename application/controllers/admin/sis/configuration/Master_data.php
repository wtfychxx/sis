...
class Master_data extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $last = end($segment);
        $data['module_table_id'] = $this->db->get_where('ci_sis_master_module_table', array('name' => $last))->row()->id;
        ...
        switch ($last) {
            ...
            case 'salutation':
                $data['breadcrumb'] = '<b> Salutation </b>';
                $data['content'] = 'admin/sis/configuration/master_data/v_salutation';
                $data['isadmin'] = $this->M_home->getUserAuth($this->session->userdata('user_id'));
                    ...
                    if ($id == 0) {
                        ...
                        foreach ($check_book_number as $row) {
                            if ($row['number'] == $this->input->post('txtinput71')) {
                                echo json_encode(array('js_form_error', array('Book Number is already exist!', 'error', 'warning')));
                                exit;
                            }
                        }
                    }
                ...
                default:
                    ...
                    if ($return_value[0] == 'js_form_insert') {
                        ...
                        $data = array(
                            ...
                            'description' => $this->input->post('txtinput[73]')
                        );
                        ...
                        $where = array();
                        trim($this->input->post('txtinput[70]')) == '' ? $where = '' : $where = array('language__id' => $this->input->post('txtinput[71]'), 'data__id' => $this->input->post('txtinput[70]'));
                        $data = array(
                            'language__id' => $this->input->post('txtinput[71]'),
                            'name' => $this->input->post('txtinput[72]'),
                            'description' => $this->input->post('txtinput[73]')
    ...
    public function ajx_delete_data($prm_id = '')
    {
        if ($this->input->is_ajax_request()) {
            if ($prm_id == '') {
                echo json_encode(array('js_form_error', array('Id cannot be empty!', 'error', 'warning')));
                exit;
            }
