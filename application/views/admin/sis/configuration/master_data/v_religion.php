...
<script>
    function get_data(){
        $('.main').show();
        $.ajax({
            url: "<?= site_url() ?>admin/sis/configuration/master_data/religion" + '/ajx_data_put',
            type: 'POST',
            dataType: 'JSON',
    ...
    $(document).ready(function(){
        ...
        $('#modalForm').on('shown.bs.modal', function(){
            ...
            loadcombo('', '<?= site_url() ?>admin/sis/configuration/master_data/religion/ajx_live_combo_put', 'language', '', '#txt71');
            if(id != ''){
                modal_data_put('modal', '<?= site_url() ?>admin/sis/configuration/master_data/religion/ajx_modal_data_put', id, language, '', 'religion');
            }
        });

        $('#show_data').on('click', '.item-delete', function(){
            var id = $(this).data('id');
            delete_data('<?= site_url() ?>admin/sis/configuration/master_data/religion/ajx_delete_data/'+id, false);
        });
        ...
        $('#btn_modal_save').on('click', function(e){
            $('#modal').validate({
                submitHandler: function(){
                    e.preventDefault();
                    form_validate('modal', '#modal', '<?= site_url() ?>admin/sis/configuration/master_data/religion/ajx_modal_insert', $('#modal').serialize(), false, '#modalForm');
                }
            });
        });
